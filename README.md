...
- **再生管理**: ワンクリックで再生済みに移動
- **削除機能**: 個別リクエストの削除
- **統一ナビゲーション**: Dashboard | All Requests | Stats | Analytics
...
#### Analytics（/analytics）
- アクセス解析ダッシュボード（今日のデータのみ、JST 6:00リセット）
  - ページビュー総数（ユーザー向けページのみ、管理画面を除く）
  - リクエスト送信数
...
- **Stats**: 統計ダッシュボード（今日のアクティビティ・累積統計・人気の曲など）
- **Analytics**: アクセス解析（今日のページビュー・コンバージョン率など、JST 6:00リセット）
...
- Dashboard、All Requests、Stats、Analyticsページは認証必須
